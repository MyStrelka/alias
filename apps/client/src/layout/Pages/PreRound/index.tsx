...
const PreRound = ({
  ...
  const teamTheme = currentTeam
    ...
    : null;
  const isSpeaker = deviceId === speaker?.deviceId;
  const isListener = deviceId === listener?.deviceId;
              ...
              !requiredReady ? (
                <button
                  onClick={() => {
                    ...
                    actions.markRoundReady(deviceId, !readyMap[deviceId]);
                  }}
                  className={`w-full py-4 rounded-xl text-xl font-bold transition-all transform active:scale-95 ${readyMap[deviceId] ? 'bg-green-600 hover:bg-green-500 text-white shadow-lg shadow-green-900/20' : 'bg-white/10 hover:bg-white/20 text-white border border-white/20'}`}
                >
                  {readyMap[deviceId] ? 'ОЖИДАНИЕ ПАРТНЕРА' : 'Я ГОТОВ!'}
                </button>
              ) : showStartButton ? (
                <button
                  onClick={() => {
